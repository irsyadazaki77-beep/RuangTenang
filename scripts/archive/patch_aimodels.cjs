const fs = require('fs');
let content = fs.readFileSync('src/lib/aiModels.ts', 'utf-8');

// I will make it export the same structure but with allowedTiers so it matches
content = `
...
fs.writeFileSync('src/lib/aiModels.ts', content);
