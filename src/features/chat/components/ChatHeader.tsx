  ...
  return (
    <header className="h-14 surface-card sticky top-0 z-20 w-full shrink-0 flex items-center justify-between px-3 sm:px-4 border-b-0 border-b">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
        {onOpenSidebar && (
          <button
            ...
            className="lg:hidden p-2 -ml-1 text-secondary hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl shrink-0 transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center"
        ...
        <div className="shrink-0 ml-1">
          {user?.role === 'guest' ? (
            <div className="flex text-xs font-medium bg-amber-50 dark:bg-amber-950/60 border border-amber-200/80 dark:border-amber-900/80 text-amber-800 dark:text-amber-300 px-2.5 py-1 rounded-full items-center gap-1 min-h-[36px]">
              <Ghost className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
              <span className="hidden sm:inline">Sesi Tamu</span>
            </div>
          ) : (
            <button
              onClick={() => setIsTemporary(!isTemporary)}
              className={`flex text-xs font-medium px-2.5 py-1.5 rounded-full items-center gap-1.5 border transition-colors cursor-pointer min-h-[36px] ${
                isTemporary
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 relative shrink-0">
        ...
        <button 
          onClick={toggleTheme} 
          className="p-2 text-secondary hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center" 
