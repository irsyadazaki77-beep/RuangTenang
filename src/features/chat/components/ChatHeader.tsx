...
import { Menu, Settings, Moon, Sun, Ghost, Shield, Brain } from 'lucide-react';
...
export function ChatHeader({
  ...
  return (
    <header className="h-12 md:h-13 surface-card sticky top-0 z-20 w-full shrink-0 flex items-center justify-between px-3 sm:px-4 border-b-0 border-b">
      {/* Left side: Mobile menu, brand logo & title, privacy badge */}
      <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
        {onOpenSidebar && (
          <button
            onClick={onOpenSidebar}
            className="lg:hidden p-1.5 -ml-1 text-secondary hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg shrink-0 transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center"
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <div className="w-6 h-6 rounded-md bg-teal-50 dark:bg-teal-950/70 border border-teal-200/80 dark:border-teal-900 shadow-3xs flex items-center justify-center p-0.5">
            <img src="/favicon.svg" alt="RuangTenang" className="w-full h-full object-contain" />
          </div>
          <span className="font-bold text-xs sm:text-sm text-primary tracking-tight hidden xs:inline">
            RuangTenang
          </span>
        </div>
        ...
        <div className="shrink-0 ml-1">
          {user?.role === 'guest' ? (
            <div className="flex text-[11px] font-medium bg-amber-50 dark:bg-amber-950/60 border border-amber-200/80 dark:border-amber-900/80 text-amber-800 dark:text-amber-300 px-2 py-0.5 rounded-full items-center gap-1 min-h-[28px]">
            ...
            >
              ...
              <span className="hidden sm:inline">{isTemporary ? 'Sementara' : 'Tersimpan'}</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 relative shrink-0">
        ...
        <button
          ...
          title="Pengaturan Percakapan & Model AI"
        >
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-teal-600 dark:text-teal-400 font-semibold">
                    {AVAILABLE_AI_MODELS.find(m => m.id === aiModel)?.tag || 'Gemini AI'}
                  </span>
                </div>
                <select
                  value={aiModel}
                  onChange={e => setAiModel(e.target.value)}
                  className="w-full text-xs font-medium surface-muted rounded-lg px-2.5 py-1.5 text-primary focus:ring-1 focus:ring-teal-500 outline-none cursor-pointer"
                >
