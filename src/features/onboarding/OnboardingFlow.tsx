...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ userId, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 shadow-xl border border-slate-100 flex flex-col justify-between min-h-[460px] max-h-[90vh] overflow-y-auto text-slate-800 relative">
        
        {/* Skip button top right */}
        <button
          onClick={handleSkip}
          className="absolute top-4 right-5 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors py-2 px-3 rounded-lg hover:bg-slate-50 cursor-pointer min-h-[36px]"
        >
          Lewati
        </button>
          ...
          {[1, 2, 3].map(s => (
            <div
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                s === step ? 'w-8 bg-teal-600' : 'w-2 bg-slate-200'
              }`}
            />
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
                  ...
                  {goals.map(goal => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left text-xs sm:text-sm transition-all cursor-pointer ${
                          isSelected
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="space-y-3.5 text-xs sm:text-sm text-slate-500 leading-relaxed">
                  <p>
        ...
        {/* Footer actions */}
        <div className="flex justify-between items-center mt-8 pt-4 border-t border-slate-50 shrink-0">
          {step > 1 ? (
            <button
              onClick={() => setStep(prev => prev - 1)}
              className="text-xs sm:text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors py-2 px-4 rounded-lg hover:bg-slate-50 cursor-pointer min-h-[44px]"
            >
              Kembali
            </button>
          ) : (
