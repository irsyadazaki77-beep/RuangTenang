...
export const AiQuotaBadge: React.FC<AiQuotaBadgeProps> = ({
  ...
  const fetchUsageStats = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (data) {
        setStats(data);
      } else if (!cachedUsageStats) {
         setError('Gagal memuat kuota AI');
      }
    } catch (err: any) {
      console.warn('Failed to update stats:', err);
      if (!cachedUsageStats) setError(err.message || 'Gagal memuat kuota AI');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (error && variant === 'card') {
    return (
      <ErrorState 
        type="network" 
        title="Gagal Memuat Kuota" 
        description={error} 
        onRetry={() => fetchUsageStats(true)} 
        className="py-4 border border-rose-200"
      />
    );
  }

  if (error) return null; // Compact/composer variants fail silently on error
  ...
  // VARIANT 2: COMPOSER / BANNER SUBTLE INDICATOR (Used above chat composer or as banner)
  if (variant === "composer" || variant === "banner") {
